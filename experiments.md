...
1. **combo_best (100K + entropy 0.05) = лучший баланс** 🏆
   - Sharpe 3.32 (ЛУЧШИЙ!)
   - Return +232.96%
   - Drawdown -9.66% (в рамках -15%)
   - Гипотеза подтверждена: комбинация дает лучшее из обоих миров

2. **high_lr (5e-4) = максимальный return, но рискованно** ⚠️
   - Return +245.71% (ЛУЧШИЙ!)
   - Win Rate 42.76% (ЛУЧШИЙ!)
   - НО Drawdown -15.10% (на грани допустимого!)
   - Высокий LR ускоряет сходимость но увеличивает риск

3. **200K timesteps = diminishing returns** 📉
   - steps_200k (+175%) < combo_best (+232%)
   - steps_200k_ent (+201%) < combo_best (+232%)
   - Больше шагов не всегда лучше! 100K + правильный entropy важнее

4. **Высокий entropy критически важен для Sharpe**
   - combo_best (entropy 0.05): Sharpe 3.32
   - steps_200k (entropy 0.01): Sharpe 2.85
   - Разница огромная при одинаковых шагах

#### Рекомендация:

**WINNER: combo_best** (100K timesteps + entropy 0.05)
- Лучший risk-adjusted return (Sharpe 3.32)
- Безопасный drawdown (-9.66%)
- Отличный outperformance (+161.53%)

---

## 🎉 MILESTONE 1 COMPLETE!

**combo_best** проходит все критерии:

| Критерий | Требование | Результат | Статус |
|----------|------------|-----------|--------|
| Outperformance | > 5% | +161.53% | ✅ PASS |
| Sharpe Ratio | > 1.0 | 3.32 | ✅ PASS |
| Max Drawdown | < -15% | -9.66% | ✅ PASS |

**Следующий шаг:** Milestone 2 - Реальные ценовые данные (yfinance/ccxt)
...
| #3 | **combo_best** | **+232.96%** | **3.32** | -9.66% | 100K + entropy 0.05 = лучший баланс |

### Лучшая конфигурация (Milestone 1 Winner)
...
```python
{
    ...
    "normalize_obs": True,
    "entropy_coef": 0.05,      # Ключ к высокому Sharpe!
    "transaction_penalty": 0.0,
    "learning_rate": 3e-4,
    "timesteps": 100000,       # 100K достаточно
}
```
